...
- [Error management configuration](#error-management-configuration)
    ...
    - [Promise.onPossiblyUnhandledRejection](#promise.onpossiblyunhandledrejection)
    - [Promise.onUnhandledRejectionHandled](#promise.onunhandledrejectionhandled)
...
.map(
    ...
    [Object {concurrency: int} options]
) -> Promise
```
...
```js
Promise.fromNode(function(function callback) resolver) -> Promise
```
...
emailTemplates(templatesDir).then(function(template) {
    return Promise.fromNode(template.bind(null, "newsletter"))
        .spread(function(html, text) {
            console.log(html, text);
        });
});
...
The [`.reflect`](.) method returns a promise that is always successful when this promise is settled. Its fulfillment value is a `PromiseInspection` instance that reflects the resolution this promise.

Using `.reflect()` to implement `settleProps` (see [Promise.settle](.) if you are unfamiliar with the array version):

```js
function settleProps(obj) {
    var ret = {};
    Object.keys(obj).map(function(key) {
        ret[key] = Promise.resolve(obj[key]).reflect();
    });
